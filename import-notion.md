...
It surveys the export, proposes a mapping from the old status vocabulary onto the six GTD statuses,
folds the old tags onto the vault's existing tag vocabulary, and only then writes item notes.
Propose-then-apply, logged, non-destructive, and nothing outside `GTD/`.

**There is no `/gtd-import` skill, on purpose.** Importing is a once-per-vault job. Installing a
command into every vault to do it — and then keeping that copy up to date forever — costs more than
pasting this page in the one time you need it.

- **No LLM-GTD yet?** Run [`install.md`](install.md) first — this prompt writes items against a
  schema that has to already be there.
- **Vault below schema v5?** `GTD/Attachments/` and the `[import]` log op arrived in v5. Run
  [`update.md`](update.md) first if you want those; the import works either way and will tell you.
...
Import an existing task system into this LLM-GTD Obsidian vault. It already has LLM-GTD installed (a `CLAUDE.md` schema, `GTD/` with a board and item notes, and the `/gtd-triage`, `/gtd-review`, `/gtd-update` skills) — read that `CLAUDE.md` first and treat it as binding. Follow everything below against my export.
...
If `CLAUDE.md` says the schema version is below 5, say so once — `GTD/Attachments/` and the `[import]` log op arrived in v5, and `update.md` adds them — then carry on regardless; the import doesn't depend on the marker.

## Ground rules

- **The export is read-only.** Never write into, move, or delete anything inside it. When the import finishes, tell me it's safe to delete — don't delete it.
- **Never overwrite an existing item note.** Collisions get reported; I decide.
- **Nothing outside `GTD/`,** with exactly one exception: unzipping the export into the staging folder `.gtd-import/` (step 1). That folder is scratch space I own, not vault content, and you say so before creating it. Items go to `GTD/Items/` (or `GTD/Archive/`), attachments to `GTD/Attachments/`. Notion pages that aren't tasks — wiki pages, meeting notes, reference docs — are **out of scope**: list them at the end so I can place them, but don't file them anywhere.
...
1. **`.gtd-import/`** — the documented drop folder. This is where I'm told to put the export; a leading dot keeps Obsidian from indexing it.
...
Say what you found and which candidate you're using. If several look plausible, list them with their sizes and dates and ask which. **Never scan my whole home directory or the whole disk** — those four locations, then ask.

If nothing turns up, don't just ask for a path — give me the short answer: *"Create a `.gtd-import/` folder at the vault root, drop the Notion zip in, and re-run — or tell me the path."*

**Staging.** If what you found is one or more `.zip` files:

- Unzip **every part** into `.gtd-import/unzipped/` (create `.gtd-import/` if needed — say so first; it's the one thing this import writes outside `GTD/`). Missing a `-Part-2.zip` means silently importing a fraction of the workspace.
...
If the export is already unzipped in a **normal (non-dot) folder inside the vault**, warn me: Obsidian is indexing those pages right now, and they'll surface in search and possibly on the board. Offer to move the folder into `.gtd-import/`. Never leave it and say nothing.
...
| `.html` files | Notion **HTML** export — stop and ask me to re-export as **Markdown & CSV**. HTML loses property typing and maps badly. |
...
Present all of the below as one proposal and **write nothing until I confirm**. Any part of it may be overridden.
...
Optionally add one tag per source database (`notion-tasks`, `notion-reading`): useful when importing several databases, noise when importing one. Propose it and let me choose.
...
- collision with a note **already in the vault** → default to **skip and report**; ask whether I'd rather suffix them or reconcile by hand
...
Ask me nothing until you've searched for the export and surveyed it; then show me the mapping before you write anything.

---

## Why this is a prompt and not a skill

Earlier versions installed a `/gtd-import` skill into the vault, fetched from this file. That
created a delivery problem with no clean fix: `/gtd-update` only refreshes a vault when the
**schema version** rises, so edits to this file never reached vaults that had already installed
the skill — and bumping the schema for a wording change is absurd.

Importing happens once per vault. A prompt you paste is always the current one, needs no install
step, no migration, and no changelog entry. Nothing here is version-stamped, so there is nothing
to keep in sync — edit this file freely.
